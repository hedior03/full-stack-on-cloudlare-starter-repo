        ...
        <DialogHeader className="text-center space-y-4">
          <div className="flex flex-col items-center space-y-2">
            <Avatar className="h-16 w-16">
              ...
              <AvatarFallback>
                {data?.user.name ? data.user.name.charAt(0).toUpperCase() : <User className="h-8 w-8" />}
              </AvatarFallback>
            </Avatar>
            <div>
    ...
    <UserProfilePopup data={user}>
      <Button variant="ghost" className="relative h-8 w-8 rounded-full">
        <Avatar className="h-8 w-8">
          ...
          <AvatarFallback>
            {user.user.name ? user.user.name.charAt(0).toUpperCase() : <User className="h-4 w-4" />}
          </AvatarFallback>
        </Avatar>
      </Button>
    </UserProfilePopup>
    ...
    <UserProfilePopup data={data}>
      <Button variant="ghost" className="flex items-center gap-3 p-2 w-full justify-start">
        <Avatar className="h-8 w-8">
          {data.user.image && <AvatarImage src={data.user.image} alt={data.user.name || "User"} />}
          <AvatarFallback>
            {data.user.name ? data.user.name.charAt(0).toUpperCase() : <User className="h-4 w-4" />}
          </AvatarFallback>
        </Avatar>
